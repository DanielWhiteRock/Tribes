 ...
 * Payment Notes
 *
 * @package     GamiPress\Points_Purchases\Custom_Tables\Payment_Notes
 * @since       1.0.0
 */
...
/**
 * Parse query args for payment notes
 *
 * @since  1.0.0
 *
 * @param string $where
 ...
 * @return string
 */
function gamipress_purchases_payment_notes_query_where( $where, $ct_query ) {

    global $ct_table;

    if( $ct_table->name !== 'gamipress_payment_notes' ) {
        return $where;
    }
...
add_filter( 'ct_query_where', 'gamipress_purchases_payment_notes_query_where', 10, 2 );